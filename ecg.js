...
function getQuery(){
    ...
    //e = changeOnShock
    if(vars['e']>-1){$("#postRhythmShockNum").val(vars['e'])}
    //f = roscOnShock
    if(vars['f']>-1){$("#roscRhythmShockNum").val(vars['f'])}
    //g = cprRhythmChangeAfter
    if(vars['g']>-1){$("#postCprTimeBox").val(vars['g'])}
    updateSettings();

    //Variables
    //h=bpm
    if(vars['h']>-1){$("#heartRateBox").val(vars['h'])}
    //i=brpm
    if(vars['i']>-1){$("#breathingRateBox").val(vars['i'])}
    //j=sats
    if(vars['j']>-1){$("#satsBox").val(vars['j'])}
    //k=vCo2
    if(vars['k']>-1){$("#co2Box").val(vars['k'])}
    //l=maxSysBP
    if(vars['l']>-1){$("#maxSysBox").val(vars['l'])}
    //n=maxDiaBox
    if(vars['m']>-1){$("#maxDiaBox").val(vars['m'])}
    //n=minSysBox
    if(vars['n']>-1){$("#minSysBox").val(vars['n'])}
    //p=minDiaBox
    if(vars['o']>-1){$("#minDiaBox").val(vars['o'])}
    updateVars();
    //p=bCo2
    if(vars['p']>-1){$("#bCo2Box").val(vars['p'])}

    //Ventilations
    if(vars['q']>-1){$("#vpmBox").val(vars['q'])}
    updateActions();
...
function spo2On(){
    ...
    if (ecgRunning==true && spo2Running==false) {
        spo2Running = true;
        $("#spo2buttonLabel").text("Spo2 on").css({"background-color": "#ffffff","color":"#000000"})
    } else {
        ...
        spo2Set = false;
        $("#spo2").text("--");
        $("#spo2buttonLabel").text("Spo2 off").css({"background-color": "","color":""});
        spo2Running = false
    }
}
function etco2On(){
    ...
    if (ecgRunning==true && etco2Running==false) {
        etco2Running = true;
        $("#etco2buttonLabel").text("etco2 on").css({"background-color": "#ffffff","color":"#000000"})
    } else {
        ...
        $("#etco2").text("--");
        $("#rr").text("--");
        $("#etco2buttonLabel").text("etco2 off").css({"background-color": "","color":""});
        etco2Running = false
    }
}
...
function drawECG() {
    ...
    if(ecgReading == true){
        if(ecgSet == false){
            ...
            console.log("setting...")
            ecgTimeout = setTimeout(function(){$("#heartRate").text(bpm)},3000);
        }
    }else{
        $("#heartRate").text("--");
    }
...
function drawSpo2() {
    ...
    if(spo2Reading == true){
        if(spo2Set == false){
            spo2Set = true;
            spo2Timeout = setTimeout(function(){$("#spo2").text(sats)},3000);
        }
    }else{
        $("#spo2").text("--");
    }
...
function drawetco2() {
    ...
    if((rhythm == "vf" && ventilating==false)||
        ...
        if(ventilating){co2 = vCo2}else{co2=bCo2}
        var co2Perc = co2/60;
        if(co2Perc>1){
            co2Perc = 1;
        }
...
function generateRhythm(wave){
    ...
    switch(wave){
        case "vt": waveArray = vtArray;
            break;
        case "asystole": waveArray = aArray;
            break;
        case "nsr": waveArray = nsrArray;
            break;
        case "pea": waveArray = nsrArray;
            break;
        case "tdp": waveArray = tdpArray;
            break;
        case "stemi": waveArray = stemiArray;
            break;
        case "af": waveArray = afArray;
            break;
        case "aflutter": waveArray = afArray;
            break;
        case "cpr": waveArray = cprArray;
            break;
        case "svt": waveArray = svtArray;
            break;
        default: waveArray =  aArray;
            break;
    }
            ...
            if(cycleState == "onlyCompressions") {
                ...
                if (compressionsCounter >= 30) {
                    ...
                    if(etco2Running) {
                        ...
                        setTimeout(function(){cycleState = "onlyCompressions"},3000);
                    }
                }
            }
        }
...
function charge() {
    if (monitorOn == true) {
        if (defibCharging == false) {
            chargingSFX.play();
            defibCharging = true;
            $("#chargeButtonLabel").text("Charging...");
...
function charged(){
    ...
    $("#chargeButtonLabel").text("Charged");
    $("#chargeBar").text("disarm");
    chargeFlash = TweenMax.to($("#chargeButtonLabel"),0.05,{backgroundColor:"#ffff00",color:"#000000",repeat:-1,repeatDelay:0.5,yoyo:true})
    chargeShockFlash = TweenMax.to($("#shockButtonLabel"),0.05,{backgroundColor:"#ffffff",color:"#ff0000",repeat:-1,repeatDelay:0.5,yoyo:true})
    if(mobileAndTabletcheck()) {
...
function recordBP(){
    if (monitorOn==true &&
        ...
        switchSFX.play();
        bprunning = true;
        $("#bpButtonP").text("Recording").css({"background-color": "#ffffff","color":"#000000"});
            ...
            if(bpCount == maxBPCount)
            {
                ...
                runBPdown = setInterval(function(){
                    ...
                    if(bpCount == maxBPCount){
                        ...
                        $("#bpButtonP").text("Record NIBP").css({"background-color": "","color":""});
                        bprunning=false;
                    }
                },1000)
            }
...
function updateSettings(){
    ...
    clearCprTimerout();
    if(rhythmChangeOnShock>roscOnShock){roscOnShock = Number(rhythmChangeOnShock+1)}
    n=0;
    shockCounter = 0;
    $("#shockDisplay").text("");
    makeQuery();
    ...
    document.onkeydown = function (e) {
        ...
        switch(e.which){
            ...
            case 32: shock();
                e.preventDefault();
                break;
            case 38: changeEnergy(10);
                e.preventDefault();
                break;
            case 40: changeEnergy(-10);
                e.preventDefault();
                break;
            case 66: recordBP();
                e.preventDefault();
                break;
            case 67: $("#cprButton").prop("checked", !$("#cprButton").prop("checked"));
                updateActions();
                e.preventDefault();
                break;
            case 68: disarm();
            ...
            case 69: etco2On();
                e.preventDefault();
                break;
            case 86: $("#ventButton").prop("checked", !$("#ventButton").prop("checked"));
                updateActions();
                e.preventDefault();
                break;
            case 73: $("#intubatedButton").prop("checked", !$("#intubatedButton").prop("checked"));
                updateActions();
                e.preventDefault();
                break;
            case 77: metronomeOn();
...
function metronomeOn(){
    if(monitorOn){
        ...
        if(ticking){
            ...
            $("#metronomeButtonLabel").text("Metronome off").css({"background-color": "","color":""})
        }else{
            if(mobileAndTabletcheck()) {
                tickSFX.load();
            }
            ...
            ticking = true;
            $("#metronomeButtonLabel").text("Metronome on").css({"background-color": "#ffffff","color":"#000000"})
        }
    }
}
